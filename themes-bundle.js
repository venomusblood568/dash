/**
 * themes-bundle.js
 * Exposes the unified theme engine on window.__DashThemes so overlay.js
 * (an IIFE, not a module) can access it without ES import syntax.
 *
 * This file is the ONLY place that duplicates data from themes.js —
 * it's a deliberate compile/copy step. To keep it in sync, run:
 *
 *   node scripts/bundle-themes.js
 *
 * which copies the resolved values from themes.js into this file.
 *
 * ─── manifest.json content_scripts load order ────────────────────
 *   "js": ["themes-bundle.js", "overlay/overlay.js"]
 * ────────────────────────────────────────────────────────────────
 */

(function () {
  // ── Canonical token objects (copied from themes.js THEMES) ──────
  // Each key matches the token names in themes.js exactly.
  const THEMES = {
    void: {
      bg: "#0d0d0f",
      bgDeep: "#0a0a0b",
  ...
  THEMES["default"] = THEMES.void; // alias
  ...
  // ── Extension var mapper (mirrors extensionVarMap in themes.js) ─
  function applyThemeToExtension(element, themeName) {
