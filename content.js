(() => {
  ...
  // ── Storage (inlined so it's guaranteed in scope) ──────────────
  ...
  function renderCommandSuggestions(filtered) {
    ...
    if (filtered.length === 0) {
      results.innerHTML = `<div id="cp-no-match">unknown command — try <span style="color:#7b6cff">/help</span></div>`;
      return;
    }
  ...
  function renderFeedback(msg, type = "success") {
    const color =
      type === "error" ? "#ff5e5e" : type === "info" ? "#7b6cff" : "#4ade80";
      ...
      <div class="cp-info-left">
        <div>
          <div>
            ...
            <span class="cp-info-version">v1.0.0</span>
          </div>
  ...
  async function executeCommand(raw) {
    ...
    if (cmd === "/add") {
      ...
      try {
        ...
        renderFeedback(
          `✓ <span style="color:#7b6cff">${esc(alias)}</span> → ${esc(normalizedUrl.replace(/^https?:\/\//, ""))} saved`,
        );
        input.value = "";
    ...
    if (cmd === "/delete") {
      ...
      renderFeedback(
        `✓ <span style="color:#7b6cff">${esc(alias)}</span> deleted`,
      );
      input.value = "";
      setTimeout(resetToSearch, 1400);
      return;
